...
const PostItem = () => {
  ...
  const handleShare = async () => {
    const shareData = {
      title: postData?.postTitle || "Check this product",
  ...
  const downPost = posts?.filter(it => it.postParentCategory === postData.postParentCategory)
  return (
    <div className="">
      <div className="min-h-screen  bg-blue-400 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div className="bg-white border-2 border-gray-200/50 rounded-xl shadow-lg p-6 md:p-8 w-full max-w-4xl flex flex-col md:flex-row gap-8 transform hover:shadow-2xl transition-all duration-300 relative z-10">
          ...
          <button
            onClick={handleShare}
            className="flex-1 absolute top-2 right-3   bg-blue-300 py-3 px-3 rounded-lg  transition-all duration-300 text-base font-semibold shadow-xl hover:shadow-lg"
          >
            <FaShareAlt />
          </button>
          {/* Product Image */}
